              ...
              <ul>
                <li v-for="(item,index) in addressListFilter" v-bind:class="{'check':checkIndex==index}" @click="checkIndex=index;selectedAddrId=item.addressId">
                  ...
                  <div class="addr-opration addr-set-default" >
                    <a href="javascript:;" class="addr-set-default-btn" v-if="item.isDefault==false" @click="setDefault(item.addressId)"><i>设为默认</i></a>
                  </div>
...
<script>
  ...
  export default{
    ...
    computed:{
      addressListFilter(){
        return this.addressList.slice(0,this.limit);//数组取多少条
      }
    },
    components:{
      NavBread,
